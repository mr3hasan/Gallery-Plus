...
var originalPhotos = JSON.parse(JSON.stringify(galleryPhotos));
var secureVaultPin = null;
async function startServer() {
  ...
  app.post("/api/photos/batch/copy", (req, res) => {
    ...
    ids.forEach((id) => {
      ...
      if (original) {
        ...
        clone.id = "p_" + Date.now() + "_" + Math.floor(Math.random() * 1e4);
  ...
  app.post("/api/photos/batch/compress", (req, res) => {
    ...
    galleryPhotos.forEach((p) => {
      if (ids.includes(p.id)) {
        ...
        const newKB = Math.max(10, origKB * compressionRatio);
        p.size = formatKBToSize(newKB);
        p.title = p.title.replace(/\s*\(Compressed.*\)/g, "");
        const ext = p.title.match(/\.[^/.]+$/) ? p.title.match(/\.[^/.]+$/)[0] : ".jpg";
  ...
  app.get("/api/photos/duplicates/scan", (req, res) => {
    ...
    for (let i = 0; i < activePhotos.length; i++) {
      ...
      for (let j = i + 1; j < activePhotos.length; j++) {
        ...
        } else if (p1.width === p2.width && p1.height === p2.height && p1.size === p2.size && p1.title.replace(/\s*-\s*Copy.*/i, "").replace(/\s*\(Compressed.*\)/i, "") === p2.title.replace(/\s*-\s*Copy.*/i, "").replace(/\s*\(Compressed.*\)/i, "")) {
          isMatch = true;
          matchType = "exact";
          reason = "Identical size & dimensions (Exact copy)";
        } else if (p1.width === p2.width && p1.height === p2.height && (p1.title.replace(/\.[^/.]+$/, "").toLowerCase().includes(p2.title.replace(/\.[^/.]+$/, "").toLowerCase()) || p2.title.replace(/\.[^/.]+$/, "").toLowerCase().includes(p1.title.replace(/\.[^/.]+$/, "").toLowerCase()))) {
          isMatch = true;
          matchType = "similar";
  ...
  app.post("/api/photos/duplicates/inject", (req, res) => {
    ...
    clone1.id = "p_dup_" + Date.now() + "_" + Math.floor(Math.random() * 1e3);
    ...
    clone1.dateAdded = new Date(Date.now() + 1e3).toISOString();
  ...
  app.post("/api/gemini/analyze", async (req, res) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              summary: { type: import_genai.Type.STRING, description: "Enhanced artistic/photographic caption." },
              suggestedTitle: { type: import_genai.Type.STRING, description: "A high-quality gallery-ready title." },
              suggestedTags: {
              ...
              curatedExif: {
                type: import_genai.Type.OBJECT,
                properties: {
                  lens: { type: import_genai.Type.STRING },
                  aperture: { type: import_genai.Type.STRING },
                  exposureTime: { type: import_genai.Type.STRING },
                  iso: { type: import_genai.Type.STRING },
                  focalLength: { type: import_genai.Type.STRING },
      ...
      if (photo) {
        ...
        photo.tags = [.../* @__PURE__ */ new Set([...photo.tags, ...aiData.suggestedTags || []])];
        if (photo.exif) {
  ...
  app.post("/api/gemini/search", async (req, res) => {
    ...
    if (!ai) {
      ...
      const results = galleryPhotos.filter((p) => !p.isInTrash).map((photo) => {
        let score = 0;
    ...
    try {
      const photosPayload = galleryPhotos.filter((p) => !p.isInTrash).map((p) => ({
        id: p.id,
        title: p.title,
